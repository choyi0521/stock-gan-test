...
class LSTMGenerator(nn.Module):
    def __init__(self,
        ...
        self.lstm = nn.LSTM(noise_dim+lcond_dim+gcond_dim, hidden_dim, n_layers, batch_first=True, dropout=dropout)
    ...
    def forward(self, noise, local_condition=None, global_condition=None):
        ...
        output = self.linear(output.contiguous().view(b*t, self.hidden_dim))
        output = output.view(b, t, self.output_dim)
        return output
...
class LSTMDiscriminator(nn.Module):
    def __init__(self,
        ...
        self.lstm = nn.LSTM(input_dim + lcond_dim + gcond_dim, hidden_dim, n_layers, batch_first=True, dropout=dropout)
